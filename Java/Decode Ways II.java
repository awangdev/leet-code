...
class Solution {
    public int numDecodings(String s) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (i >= 2 && validate(ss[i - 1]) && validate(ss[i - 2])) {
    ...
    private int calcDoubleDigitVariation(char a, char b) {
        ...
        // Validate all conditioins of char a
        if (a == '1') return b == '*' ? 9 : 1; // 1 pair with [1~9] or one of [0~9]
        if (a == '2') { // 2 pair with [1~6] or one of [0~6]
            if (b == '*') return 6;
            if (b <= '6') return 1;
            return 0;
        }
        
        // Validate char b. Now a = '*'
