    ...
    public boolean isNumber(String s) {
        ...
        for (; i < s.length(); i++) {
            ...
            if (Character.isDigit(c)) isNum = true;
                ...
                if (isDot || isExp) return false; // `.` exists, or it is part of e, return false
                isDot = true;
            } else if (c == 'e') {
                if (isExp || !isNum) return false; // `e` exists, or if no integer ahead of e, return false
                isExp = true;
                isNum = false; // start checking integer suffix  to `e`
            } else if (c == '+' || c == '-') {
                if (i >= 1 && s.charAt(i - 1) != 'e') return false;
            } else return false; // all other characters
        }
